...
class AT_Loader {
	...
	public function model($model){
		...
		if (empty( $this->_models[$model] ) ) {
			if ( file_exists( AT_DIR . '/models/' . strtolower( $model ) . '.php' ) )	{
				$this->_models[$model] = 'AT_' . $class_name;
				include AT_DIR . '/models/' . strtolower( $model ) . '.php';
			} else {
				return null;
			}
		}
	...
	public function library($library, $object = true, $params = array()){
		if ( empty( $this->_libraries[$library] ) ) {
			if ( file_exists( AT_DIR . '/libraries/' . strtolower( $library ) . '.php' ) )	{
				$this->_libraries[$library] = 'AT_' . $library;
				include AT_DIR . '/libraries/' . strtolower( $library ) . '.php';
			} else {
				return null;
			}
		}
	...
	public function widget($widget, $params = array()){
		if ( empty( $this->_widgets[$widget] ) ) {
			if ( file_exists( AT_DIR . '/widgets/' . strtolower( $widget ) . '.php' ) )	{
				$this->_widgets[$widget] = 'AT_' . $widget;
				include AT_DIR . '/widgets/' . strtolower( $widget ) . '.php';
			} else {
				return null;
			}
		}
	...
	public function helper($helper, $object = false){
		if ( empty( $this->_helpers[$helper] ) ) {
			if ( file_exists( AT_DIR . '/helpers/' . strtolower( $helper ) . '.php' ) )	{
				$this->_helpers[$helper] = 'AT_' . $helper;	
				include AT_DIR . '/helpers/' . strtolower( $helper ) . '.php';
			}
		}
		if ($object) {
			$helper = 'AT_' . $helper;
